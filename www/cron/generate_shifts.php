...
require_once '/home2/elcerrit/elcerritovalle.org/MUNAY/www/api/config.php';
...
function generateShiftsForWeek($startDate = null) {
    ...
    foreach ($caregivers as $caregiver) {
        ...
        foreach ($weekDays as $date) {
            $check = $pdo->prepare("SELECT id FROM shifts WHERE caregiver_id = ? AND shift_date = ? AND shift_type = 'base'");
