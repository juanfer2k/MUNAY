...
$user_role = $_SESSION['user_role'] ?? 'caregiver';
$page_title = match($user_role) {
    'admin' => 'Panel de Administración',
    'caregiver' => 'Mis Turnos',
...
.admin-panel .badge-caregiver { background: #1E8449; color: #fff; }
...
<div class="admin-panel">
    ...
    function abrirModalTurno(t) {
        ...
        fetch(BASE_URL + 'api/users.php?role=caregiver')
            .then(r => r.json())
            .then(d => {
    ...
    // ============================================================
    //  FUNCIONES PARA CUIDADOR (CORREGIDAS)
    // ============================================================
    ...
    // ============================================================
    //  FUNCIONES PARA POLICÍA (simplificadas)
    // ============================================================
    ...
    function renderDashboard() {
        ...
        contentContainer.innerHTML = tabs.map((tab, index) => {
            ...
            switch(tab.id) {
                case 'users':
                    columns = ['ID', 'Nombre', 'Email', 'Teléfono', 'Rol', 'Grupo', 'Acciones'];
                    tableId = 'usersTable';
                    listId = 'usersList';
                    addButton = '<button onclick="abrirModalUsuario()" class="btn btn-primary">+ Nuevo</button>';
                    break;
                case 'shifts':
                    columns = ['ID', 'Cuidador', 'Fecha', 'Horario', 'Ubicación', 'Estado', 'Acciones'];
                    tableId = 'shiftsTable';
                    listId = 'shiftsList';
